...
test.describe('iGraSpore V2 — Canonical Regression', () => {
  ...
  test('[C04] Player grows (size increases after eating)', async ({ page }) => {
    await startGame(page);
    const before = await page.evaluate('+player.size.toFixed(3)');
    // Feed player 10 times
    await page.evaluate(`
      for (var i = 0; i < 10; i++) {
        var prey = null, best = 1e9;
    ...
    `);
    const after = await page.evaluate('+player.size.toFixed(3)');
  ...
  test('[R02] Realistic mode toggle', async ({ page }) => {
    await startGame(page);
    await page.evaluate('if(typeof toggleRenderModeLarge==="function") toggleRenderModeLarge();');
    await page.waitForTimeout(500);
    const mode = await page.evaluate('settings.renderMode');
