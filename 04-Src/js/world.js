  ...
  for(var cat in INIT_N){
    ...
    for(var s=0; s<selectedSpecies.length; s++) {
       ...
       for(var i=0;i<baseCnt;i++){
         var d=(cat==='producer')?rng(2,80):rng(15,PD*0.6);var hw=Math.max(40,halfW(d)-40);
    ...
    for(var cat in TGT){
      ...
      if(bm < TGT[cat]*avgBm*DIFF[difficulty].spawn*settings.density){
        ...
        for (var k=0; k<numToSpawn; k++) {
           if(pool.length>0){var sp=pool[Math.floor(Math.random()*pool.length)];var d=(cat==='producer')?rng(2,80):rng(15,PD*0.6);var hw=Math.max(40,halfW(d)-30);spawnOrg(sp,rng(-hw,hw),d);}
        }
      }
    }
  }
