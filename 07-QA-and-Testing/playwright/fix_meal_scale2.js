...
test('final meal scale', async ({ page }) => {
  ...
  const meals = await page.evaluate(() => {
    ...
    function oneMeal(sz, en){
      player.energy=40; player.massFood=0; player.stomach=[]; player.size=player.sp.size;
      ...
      return {dEn:+(player.energy-e0).toFixed(2), dMass:+(player.massFood-m0).toFixed(3),
              lastEn:+(player._lastEnGain||0).toFixed(2), lastM:+(player._lastMassGain||0).toFixed(3)};
    }
    const s=oneMeal(0.8, 15);
    const m=oneMeal(2.5, 50);
    const l=oneMeal(5.0, 100);
    return {s,m,l,
      okEn: l.dEn>m.dEn && m.dEn>s.dEn,
      okMass: l.dMass>m.dMass && m.dMass>s.dMass};
  });
  console.log('MEALS '+JSON.stringify(meals));

  const gate=await page.evaluate(()=>{
    player.eatsSinceDiv=1; player.energy=player.sp.repEnergy+40; player.age=90;
    player.divCD=0; player.dividing=false; player.size=player.sp.size*1.05; player.massFood=30;
    return {can:canDivide(player), reason:divideBlockReason(player)};
  });
  console.log('GATE '+JSON.stringify(gate));

  const sun=await page.evaluate(()=>{
    const sp=SPECIES_DB.find(s=>s.cat==='producer');
    const o=spawnOrg(sp,0,PD*0.1,false); o.massFood=0; o.energy=45;
    try{dayLight=1;}catch(e){}
    const m0=o.massFood,e0=o.energy;
    for(let i=0;i<40;i++) updateOrg(o,0.05);
    return {dMass:+(o.massFood-m0).toFixed(2), dEn:+(o.energy-e0).toFixed(2)};
  });
