...
function updateWorld(dt){
  if(dt>0.05) dt=0.05; // cap raw frame (~20fps) BEFORE speed so 10x stays 10x
  dt*=timeScale;
...
// Chemotaxis update
  ...
  if(popHist.length<2 || fc%90===0){var snap={};for(var cat in CC){var c=0;for(var j=0;j<orgs.length;j++)if(orgs[j].alive&&orgs[j].sp.cat===cat)c++;snap[cat]=c;}popHist.push(snap);if(popHist.length>100)popHist.shift();}
}

function updateCamera(dt){
  // Clamp dt to prevent camera jumps on frame drops
  var _tsCam=(typeof timeScale==='number' && timeScale>0)?timeScale:1;
  var dtc=clamp(dt,0,0.05*Math.max(1,_tsCam));
