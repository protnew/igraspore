...
function renderSky(vL,vR,vT){
  ...
  if((dl < 0.55 || t >= 16.5 || t < 6) && !(typeof zoom==='number' && zoom>2.0)){
    ctx.save();
  ...
  if(dl > 0.3 && window.skyClouds && !(typeof zoom==='number' && zoom>2.2)){
    ctx.save();
  ...
  if((dl > 0.04 || (t>=16.5 && t<21.2)) && t > 4.5 && t < 21.2){
    ...
    // microbe zoom: world bloom reads as circular air glares — overlay draws the sun
    if(z > 2.2){
