...
function render(){
  ...
  // === STEP 1: BACKGROUND ===
  if(window.HYBRID_GL){
    // V3: WebGL renders water background — canvas is transparent overlay
    ctx.clearRect(0,0,cv.width,cv.height);
  } else {
    // V2: Canvas draws sky+water background
    var skyFrac = 0;
  ...
  // === STEP 3: ENVIRONMENT ===
  if(window.HYBRID_GL){
    // V3: WebGL handles water/sky/sun. Only draw overlays on canvas.
    if(!isReal) renderNutrients(vL,vR,vB);
