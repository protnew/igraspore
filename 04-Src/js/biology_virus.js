...
function updateViruses(dt){
  ...
  for(var i=viruses.length-1;i>=0;i--){
    ...
    // Brownian drift — phages diffuse in water even without target
    if(!v.target||!v.target.alive){
      v.vx+=(Math.random()-0.5)*0.3;
      v.vy+=(Math.random()-0.5)*0.3;
    }
    // Speed clamp — keep phages moving but not too fast
    var vsp=Math.sqrt(v.vx*v.vx+v.vy*v.vy);
    if(vsp>2.5){v.vx=v.vx/vsp*2.5;v.vy=v.vy/vsp*2.5;}
    if(vsp<0.3){v.vx=v.vx/(vsp||1)*0.3;v.vy=v.vy/(vsp||1)*0.3;}
    v.vx*=0.96;v.vy*=0.96;
...
function spawnVirus(){
  if(viruses.length>120)return; // raised cap: more viruses visible
  ...
  viruses.push({x:rng(-hw,hw),y:Math.max(20,Math.min(PD-20,d)),vx:rng(-0.8,0.8),vy:rng(-0.8,0.8),
