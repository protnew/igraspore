...
window.drawBioicon=function(ctx,o,sz,sh){
  ...
  if(col.indexOf('rgb')>=0){var m=col.match(/(\d+)/g);if(m&&m.length>=3){
    ...
    col='#'+hr+hg+hb;}}
  /* PHASE-CONTRAST: desaturate species color toward gray — microscopy look, not cartoon */
  try{
    var cr=parseInt(col.substr(1,2),16),cg=parseInt(col.substr(3,2),16),cb=parseInt(col.substr(5,2),16);
    var gray=(cr*0.3+cg*0.59+cb*0.11);
    var DESAT=0.82; /* 82% toward gray */
    cr=Math.round(cr+(gray-cr)*DESAT); cg=Math.round(cg+(gray-cg)*DESAT); cb=Math.round(cb+(gray-cb)*DESAT);
    var h2=function(v){var s=v.toString(16);return s.length<2?'0'+s:s;};
    col='#'+h2(cr)+h2(cg)+h2(cb);
  }catch(e){}
